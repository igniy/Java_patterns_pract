...
public class ScheduleService {
    ...
    public void scheduleFixedDelayTask() throws Exception {
        ...
        String url = "jdbc:postgresql://localhost:5432/practJava";
        String password = "postgres";
        String username = "postgres";

        Connection connection = DriverManager.getConnection(url, username, password);

        fetchDataFromDatabase("SELECT * FROM building", connection);
        String csvFilePathForStudent = "src/main/myTests/1.csv";
        printToCsv(resultSetArray, csvFilePathForStudent);

        fetchDataFromDatabase("SELECT * FROM adress", connection);
